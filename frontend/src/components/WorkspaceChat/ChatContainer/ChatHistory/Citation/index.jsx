  ...
  return (
    <div className="flex flex-col mt-4 justify-left">
      <button
        ...
        className={`text-theme-text-muted font-medium italic text-sm text-left ml-14 pt-2 ${
          open ? "pb-2" : ""
        } hover:text-theme-text transition-all duration-300`}
      >
      ...
      <div className="w-full max-w-2xl bg-main-gradient rounded-lg shadow border border-white/10 overflow-hidden">
        <div className="relative p-6 border-b rounded-t border-gray-500/50">
          <div className="w-full flex gap-x-2 items-center">
            {isUrl ? (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="text-xl font-semibold text-theme-text overflow-hidden overflow-ellipsis whitespace-nowrap hover:underline hover:text-blue-300 flex items-center gap-x-1"
              >
              ...
              <h3 className="text-xl font-semibold text-theme-text overflow-hidden overflow-ellipsis whitespace-nowrap">
                {truncate(title, 45)}
              </h3>
            )}
          </div>
          ...
          <button
            onClick={onClose}
            type="button"
            className="absolute top-6 right-6 transition-all duration-300 text-theme-text-muted bg-transparent hover:border-outline rounded-lg text-sm p-1.5 inline-flex items-center bg-sidebar-button hover:bg-menu-item-selected-gradient hover:border-slate-100 hover:border-opacity-50 border-transparent border"
          >
        ...
        >
          <div className="p-6 space-y-2 flex-col">
            ...
            {chunks.map(({ text, score }, idx) => (
              <div key={idx} className="pt-6 text-theme-text">
                <div className="flex flex-col w-full justify-start pb-6 gap-y-1">
                  <p className="text-theme-text whitespace-pre-line">
                    {HTMLDecode(omitChunkHeader(text))}
                  </p>

                  {!!score && (
                    <>
                      <div className="w-full flex items-center text-xs text-theme-text-muted gap-x-2 cursor-default">
                        <div
