...
const QWEN_BASE_PATH = "https://dashscope-intl.aliyuncs.com/compatible-mode/v1";
const QWEN_MODELS = [
...
export default function QwenOptions({ settings }) {
  ...
  const modelPref = QWEN_MODELS.some((m) => m.id === settings?.GenericOpenAiModelPref)
    ? settings.GenericOpenAiModelPref
    : "qwen-turbo";
  const tokenLimit = settings?.GenericOpenAiTokenLimit || 8000;
          ...
          <div className="flex gap-2 items-end">
            <div className="flex-1 relative">
              <input
                ...
                placeholder="Alibaba DashScope API Key"
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  const isMasked =
                    ...
                    /^\*+$/.test(key) ||
                    key === "*".repeat(20);
                  const result = await System.validateAiApiKey(
                    "qwen",
                    isMasked ? "" : key,
                    QWEN_BASE_PATH
                  );
