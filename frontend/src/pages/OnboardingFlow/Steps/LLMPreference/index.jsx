...
export default function LLMPreference({
  ...
  const handleSubmit = async (e) => {
    ...
    const selectedProvider = LLMS.find((llm) => llm.value === selectedLLM);
    const backendProvider = selectedProvider?.backendValue ?? selectedLLM;
    data.LLMProvider = backendProvider;
  ...
  return (
    <div>
      <form ref={formRef} onSubmit={handleSubmit} className="w-full">
        <div className="w-full relative border-outline shadow-lg border-2 rounded-xl bg-white/60 backdrop-blur-sm">
          <div className="w-full p-4 absolute top-0 rounded-t-xl">
            <div className="w-full flex items-center sticky top-0">
              <MagnifyingGlass
                size={16}
                weight="bold"
                className="absolute left-4 z-30 text-theme-text"
              />
              <input
                type="text"
                placeholder="Search LLM providers"
                className="bg-dropdown-bg z-20 pl-10 h-[38px] rounded-full w-full px-4 py-1 text-sm border-2 border-outline outline-none focus:border-outline focus:ring-2 focus:ring-outline text-theme-text placeholder:text-theme-text-muted"
